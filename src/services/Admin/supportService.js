  ...
  const result = await pool.query(
    `SELECT COUNT(*) AS unread_count 
     ...
     WHERE sr.status = 'open' 
        OR (
          sr.status = 'in_progress' AND 
          (SELECT is_admin_reply FROM support_responses WHERE request_id = sr.id ORDER BY created_at DESC LIMIT 1) = false
        )`,
  );
...
export const getAllTicketsService = async (filters) => {
  ...
  let query = `
    ...
    CASE 
      WHEN sr.status = 'open' THEN true
      WHEN sr.status = 'in_progress' AND (
          SELECT is_admin_reply FROM support_responses WHERE request_id = sr.id ORDER BY created_at DESC LIMIT 1
      ) = false THEN true
      ELSE false
  ...
  if (!sort_by) {
    ...
    const sortColumn =
      ALLOWED_SORT_COLUMNS[sort_by] || "COALESCE(sr.updated_at, sr.created_at)";
    const sortDirection =
  ...
  let statsQuery = `
    SELECT 
      ...
      COUNT(*) FILTER (
        ...
        (status = 'in_progress' AND (SELECT is_admin_reply FROM support_responses WHERE request_id = sr.id ORDER BY created_at DESC LIMIT 1) = false)
      ) as unread,
...
export const replyTicketService = async (
  ...
  try {
    ...
    if (ticketRes.rows.length === 0)
      throw new Error("Không tìm thấy vé hỗ trợ");
...
export const updateTicketStatusAdminService = async (
  ticketId,
  adminId,
  newStatus,
) => {
  ...
  try {
    ...
    if (currentStatus === newStatus) {
      throw new Error(
        "Trạng thái mới không được trùng với trạng thái hiện tại!",
      );
    }
    ...
    await client.query(
      ...
      [
        ticketId,
        adminId,
        `Hệ thống: Quản trị viên đã thay đổi trạng thái vé thành "${statusText}".`,
      ],
    );
    ...
    if (userId) {
      await client.query(
        ...
        [
          userId,
          "Cập nhật trạng thái vé hỗ trợ",
          `Vé hỗ trợ #${ticketRes.rows[0].ticket_code} của bạn đã chuyển sang trạng thái: ${statusText}.`,
          "support",
          ticketId,
        ],
      );
